...
2. for 문을 이용하여 strArr의 길이만큼 반복시켜 모든 인덱스를 거친다.
3. if문과 나머지 연산자를 이용하여 strArr 내부의 인덱스의 2로 나눈 나머지가 1인 경우(짝수 번째) 해당하는 인덱스의 문자열의 문자를 소문자로 변경하기 위해 for문을 이용, answer에 추가한다.
4. 인덱스의 나머지가 1이 아닌 경우 for문을 이용하여 해당 인덱스의 문자열의 모든 문자를 대문자로 변경하여 answer에 추가한다.
5. return answer
